...
// number — and the server only returns the expense that id points at.
...
export default function EditExpensePage({
  ...
  return (
    <div className="flex w-full flex-col">
      <PageToolbar icon={Calculator01Icon} title={t().expenses.editTitle} />
      <div className="p-4">
        {/* key={id} remounts a fresh boundary when navigating between ids */}
        <QueryErrorBoundary
          key={id}
          fallbackTitle={t().expenses.notFoundTitle}
          fallbackBody={t().expenses.notFoundBody}
        >
          <ExpenseLoader id={id} />
        </QueryErrorBoundary>
      </div>
...
function ExpenseLoader({ id }: { id: string }) {
  ...
  const expense = useQuery(
    api.expenses.get,
    user == null || !validId ? "skip" : { expenseId: id as Id<"expenses"> },
  );

  if (!validId || expense === null) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>{t().expenses.notFoundTitle}</CardTitle>
          <CardDescription>{t().expenses.notFoundBody}</CardDescription>
        </CardHeader>
      </Card>
    );
  }
  ...
  return <ExpenseForm expense={expense} onDone={() => router.push("/expenses")} />;
}
